...
def oracle_sharpe_by_complexity(
    mu: np.ndarray,
    w_star: np.ndarray,
    mean: np.ndarray,
    c_grid: np.ndarray,
...
def learned_sharpe_curves(
    ...
    mu, w_star, mean, sigma = spectral_dgp(
        n=n, b=b, r=r, h_decay=h_decay, target_sr=target_sr
    )
    ...
    # We draw the sample mean directly from its exact Gaussian sampling law.
    for t in t_values:
...
def in_sample_vs_out_of_sample(
    t: int,
    reps: int,
    n: int = 150,
    b: float = 1.10,
    ...
    """Compare genuine in-sample fit with population OOS performance.

    For every spectral cutoff C, estimate the exact empirical response-one
    optimum on the first C eigendirections.  Because the classes are nested,
    in-sample Sharpe is monotone.  Population Sharpe is evaluated under the
    known DGP and can peak at an interior C.
    """
    mu, _w_star, mean, sigma = spectral_dgp(
        n=n, b=b, r=r, h_decay=h_decay, target_sr=target_sr
    )
    ...
    for q in range(reps):
        ...
        for k, c in enumerate(c_grid):
            ...
            # Exact empirical response-one optimum in the C-dimensional class.
            w_hat = np.linalg.solve(
                second_hat + 1e-10 * np.eye(c),
                mean_hat,
            )
...
def figure_regret_decomposition(outdir: Path, b: float, r: float) -> None:
    br = b * r
    t = 120
    a0 = 1.2
    b0 = 1.0
    ...
    ax.annotate(
        r"$C_T^\star$",
        xy=(c_star, np.interp(c_star, c, total)),
...
def figure_scaling_law(
    ...
    empirical_slope, empirical_intercept = np.polyfit(
        np.log(t_values), np.log(c_star), 1
    )

    # With h_j ~ j^{-h_decay}, approximation tail is asymptotically
    # C^{-alpha}, alpha = b r + 2 h_decay - 1.
    alpha = b * r + 2.0 * h_decay - 1.0
    theory_slope = 1.0 / (alpha + 1.0)

    theory_level = np.exp(
        np.mean(np.log(c_star) - theory_slope * np.log(t_values))
    )
...
def figure_in_vs_out(outdir: Path, reps: int, b: float, r: float, h_decay: float):
    ...
    return c_grid, sr_in, sr_out


def figure_nominal_vs_effective(outdir: Path, b: float) -> None:
...
def write_summary(
    ...
    lines = [
        "T,C_star",
        *[f"{int(t)},{int(c)}" for t, c in zip(t_values, c_star)],
    ]
...
def main() -> None:
    ...
    parser.add_argument(
        "--quick",
        action="store_true",
        help="Use fewer Monte Carlo replications for a fast smoke test.",
    )
    ...
    paper_style()
    figure_regret_decomposition(args.outdir, b=b, r=r)
    figure_oracle_vs_learned(args.outdir, reps=reps, b=b, r=r, h_decay=h_decay)
    t_values, c_star, empirical_slope, theory_slope = figure_scaling_law(
        args.outdir, reps=reps, b=b, r=r, h_decay=h_decay
    )
    figure_in_vs_out(args.outdir, reps=reps, b=b, r=r, h_decay=h_decay)
    figure_nominal_vs_effective(args.outdir, b=b)
    write_summary(
        args.outdir,
        t_values=t_values,
        c_star=c_star,
